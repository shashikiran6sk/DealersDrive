...
export function tokenSignature(text, signatureFileName = 'sig.tsx') {
  ...
  const walk = (node) => {
    ...
    /*
     * A JSX container with no expression holds only a comment. A prose one is
     * removed by this migration and renders nothing either way, so it is skipped
     * on both sides; one holding a directive survives and has to be counted.
     */
    if (ts.isJsxExpression(node) && node.expression === undefined) {
