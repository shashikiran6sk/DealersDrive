...
| ----- | ------------------------------------ | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- | ----------------------------------------- |
| C040  | `OnboardingWizard`                   | `features/auth/onboarding-wizard.tsx:57`    | `step`, `session`, `documents`, `dealer`, `completeness`, `yardPhoto`, `phoneWidget` (**R39**) — ⚠️ `cities` removed by **D6** | 4 steps × valid/invalid/submitting, name-taken, business-with-tagline-and-services, changes-requested (×2), step 1 unproved / proved (**R39**)                                                               | F037–F043 | **P0** ✅                                 |
| C040b | `PhoneVerification`                  | `features/auth/phone-verification.tsx`      | `widget: PhoneOtpWidget \| null`, `phone`, `fullName`, `verified`, `onVerified`, `onContinue`, `onBeforeSend`, `initialStage?` | send code, code entry, refused (attempts left), attempts spent, verified, not configured, service unreachable                                                                                                | R39       | **P0** ✅                                 |
...
| C062  | `DealerAdminActions`                 | `features/admin/dealer-actions.tsx:36`      | `dealer: AdminDealerDetail`                                                                                                    | per dealer status × approve (typed action + dealership name, KYC required) × request changes × reject (disclosure + name confirmation) × suspend form × reinstate × pending × error — grant form at **F054** | F045      | **P0** ✅                                 |
