...
## `apps/sandbox/src/stories/search/dealer-search-box.stories.tsx`
...
_pause_, not one per keystroke. Seven characters typed in one go is one
...
the dealership is offered because its _town_ matched, so the name carries
