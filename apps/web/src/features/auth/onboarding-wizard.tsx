...
export function OnboardingWizard({
  ...
  /**
   * What the browser needs to ask Firebase for an OTP (**R39**), read on the
   * server from `GET /v1/config/public` and handed down as a prop.
   *
   * Not a `NEXT_PUBLIC_*` (rule 9): those are inlined at build time, and one
   * image per environment is exactly what build-once-promote-many exists to
   * avoid. `null` is a first-class state — a deployment running the `fake`
   * driver has no project, and step 1 says so rather than failing silently.
   */
