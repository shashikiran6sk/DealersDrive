 ...
 *
 * `PHONE_VERIFICATION_DRIVER=fake` is what the suite runs on: the token is a
 * structured string rather than a Firebase ID token, so no message is sent, no
 * Firebase project is needed and nothing waits on a network. Everything above
 * the verifier runs exactly as it does in production — the same route, the same
 * duplicate check, the same write.
 */
...
describe('onboarding', () => {
   ...
   * the dealer and cheaper — Firebase's free tier is a daily message count.
   *
  ...
  it('refuses a phone number another dealership already uses', async () => {
    ...
    const late = await agent
      .post('/v1/auth/phone/verify')
      .send({ idToken: 'fake:+919840012345' })
      .expect(409);
    expect(late.body.code).toBe('PHONE_ALREADY_REGISTERED');
  });
